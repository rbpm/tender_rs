...
fn main() {
    ...
    let mut offer_book = new_file_empty_worksheet();
    let mut all_tender_book = new_file_empty_worksheet();
    let mut all_offer_book = new_file_empty_worksheet();
    ...
    offer_book = process(offer_book, &order_data, IT_ORDERS, &args, false);
    all_offer_book = process(all_offer_book, &order_data, ORDERS, &args, true);
    let _ = writer::xlsx::write(&tender_book, std::path::Path::new(IT_TENDERS_FILENAME));
    let _ = writer::xlsx::write(&offer_book, std::path::Path::new(IT_ORDERS_FILENAME));
    let _ = writer::xlsx::write(&all_tender_book, std::path::Path::new(TENDERS_FILENAME));
    let _ = writer::xlsx::write(&all_offer_book, std::path::Path::new(ORDERS_FILENAME));
}
